...
const Keunggulan = () => {
    const showcaseSteps = [
        {
            icon: <FaHardHat className="text-[#86CEEB] text-2xl" />,
    ...
    return (
        <section id='keunggulan'>
            <div className='lg:mx-14 md:mx-10 mx-5 border-2 border-sky-400 border-dashed p-4 rounded-xl mt-10 lg:mt-0 text-justify'>
                <p className='text-sky-700 text-base lg:text-lg font-bold mt-3 lg:mt-5 text-center'>KEUNGGULAN WEBSITE</p>
                ...
                <div className="grid md:grid-cols-3 gap-8 lg:gap-4 my-10 lg:mt-10">
                    {showcaseSteps.map((step, index) => (
                        <div
                            key={index}
                            className="relative max-w-sm p-6 bg-white border-2 border-gray-300 rounded-xl hover:border-[#86CEEB] pt-14 text-center"
                        >
                            <div className="absolute -top-6 left-1/2 transform -translate-x-1/2 bg-[#E6F4FA] p-4 rounded-md shadow-md">
                                {step.icon}
                            </div>
                            <h5 className="mb-2 text-xl md:text-lg font-medium tracking-tight">{step.title}</h5>
                            <p className="text-sm font-reguler text-[#424242] mt-5">{step.desc}</p>
                        </div>
                    ))}
                </div>
            </div>
