  ...
  return (
    <div className="flex flex-col px-6 md:px-20 md:flex lg:flex h-screen items-center font-sans">
      <p className="text-right w-full font-normal mt-6 mb-5 text-md text-[#424242] dark:text-slate-300">
        Belum punya akun?
        <span onClick={handleRegister} className="text-[#0284C7] dark:text-sky-500 font-medium cursor-pointer">
          {' '}
          Daftar
        </span>
      </p>
        ...
        {options.map((item) => (
          <div
            ...
            className={`flex items-start gap-5 p-5 rounded-xl cursor-pointer border transition-all ${
              selectedOption === item.id ? 'border-[#2FA4D3] dark:border-[#525252] bg-[#EBFBFF] dark:bg-[#24373E] ' : 'border-gray-300 dark:border-[#525252] bg-white dark:bg-[#1D232A]'
            }`}
          >
